...
async def _(event):
    ...
    packname = f"@Sudanese_otaku kanged Pack"
    packshortname = f"Sudanese_otaku_kanged"  # format: Uni_Borg_userid

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@sudanese_otaku_Sticker.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"@Sudanese_otaku's Animated Stickers"
        packshortname = f"Sudanese_otaku_animated"  # format: Uni_Borg_userid
    ...
    await event.reply("Processing this sticker. Please Wait!")
    ...
    await event.reply(f"sticker added! Your pack can be found [here](t.me/addstickers/{packshortname})")


@borg.on(admin_cmd(pattern="stickerinfo"))
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
    ...
    if event.reply_to_msg_id:
        ...
        while 1:
            ...
            if not pending_tasks:
                break
        await event.reply("Downloading to my local completed")
        # https://gist.github.com/udf/e4e3dbb2e831c8b580d8fddd312714f7
        ...
        await event.reply("TODO: Not Implemented")
